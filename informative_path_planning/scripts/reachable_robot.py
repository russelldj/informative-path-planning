...
class MCTS_Reachable:
    ...
    def rollout_policy(self, node):
        ...
        for i in range(self.rl):
            actions = (
                self.goals
            )  # self.path_generator.get_path_set(self.tree[node][0][-1]) #plan from the last point in the sample
            try:
    ...
    def get_reward(self, sequence):
        ...
        if self.f_rew == "mes":
            return self.aquisition_function(
                time=self.t, xvals=obs, robot_model=sim_world, param=self.max_val
            )
...
class Reachable_Robot:
    ...
    def choose_destination(self, t):
        ...
        for i, goal in enumerate(self.goals):
            ...
            paths[i] = xvals
            value[i] = self.aquisition_function(
                time=t, xvals=xvals, robot_model=self.GP, param=param
            )
        try:
            return (
                self.goals[max(value, key=value.get)],
    ...
    def take_step(self, goal):
        ...
        if dist > self.step_size:
            new_goal = (
                self.loc[0] + self.step_size * np.sin(np.pi / 2 - angle_to_goal),
                self.loc[1] + self.step_size * np.sin(angle_to_goal),
                angle_to_goal,
            )
        else:
            new_goal = (goal[0], goal[1], angle_to_goal)
...
if __name__ == "__main__":
    ...
    world = il.Environment(
        ranges=ranges,  # x1min, x1max, x2min, x2max constraints
        NUM_PTS=20,
        variance=100.0,
        lengthscale=1.0,
        visualize=True,
