...
class Evaluation:
    ...
    def plot_metrics(self):
        ...
        mes_reward_robot = np.cumsum(
            np.array(list(self.metrics["mes_reward_robot"].values()))
        )
        mes_reward_omni = np.cumsum(
            np.array(list(self.metrics["mes_reward_omni"].values()))
        )
        ...
        for i in range(0, self.num_stars):
            ...
            star_obs_loc_x.append(
                np.array(list(self.metrics["star_obs_loc_x_" + str(i)].values()))
            )
            star_obs_loc_y.append(
                np.array(list(self.metrics["star_obs_loc_y_" + str(i)].values()))
            )
