...
parser.add_argument(
    ...
    "--reward",
    action="store",
    help="Reward function. Should be mes, ei, or info_gain.",
    default="naive_value",
)
parser.add_argument(
    "-p",
    "--pathset",
    action="store",
    help="Action set type. Should be dubins, ...",
    default="dubins",
)
parser.add_argument(
    "-t",
    "--tree",
    action="store",
    help="If using nonmyopic planner, what kind of tree serach. Should be dpw or belief.",
    default="dpw",
)
parser.add_argument(
    "-n",
    "--nonmyopic",
    action="store_true",
    help="Run planner in nonmyopic mode if flag set.",
    default=False,
)
parser.add_argument(
    "-c",
    "--cost",
    action="store_true",
    help="Divide reward of action by cost if flag set.",
    default=False,
)
parser.add_argument(
    "-g",
    "--goal",
    action="store_true",
    help="Consider the reward of final point only if flag set.",
    default=False,
)
...
if DIM == 2:
    ...
    data = np.vstack(
        [x1observe.ravel(), x2observe.ravel(), 0 * np.ones(len(x1observe.ravel()))]
    ).T
